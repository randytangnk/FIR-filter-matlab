function [ Y ] = BSF( omega_l, omega_h, X, N  )
% BSF band stop filter using Blackman Window
% 'omega_l' is the lower cutoff radian frequency
% 'omega_h' is the lower cutoff radian frequency
...
%BSF temporal response
h = (sin(omega_l*(n-(N+1)/2)) + sin(pi*(n-(N+1)/2)) - sin(omega_h*(n-(N+1)/2)))/pi./(n-(N+1)/2); % Move the symetric impulse to the positive part of the time axis 
h((N+1)/2) =1+ (omega_l-omega_h)/pi; %让其不为NAN
