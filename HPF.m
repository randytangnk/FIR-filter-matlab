function [ Y ] = HPF( omega, X, N )
% HPF highband pass filter using Blackman Window
% 'omega' is the cutoff radian frequency.
...
h = (sin(pi*(n-(N+1)/2)) - sin(omega*(n-(N+1)/2)))/pi./(n-(N+1)/2); % Move the symetric impulse to the positive part of the time axis 
h((N+1)/2) = 1-omega/pi; %让其不为NAN
